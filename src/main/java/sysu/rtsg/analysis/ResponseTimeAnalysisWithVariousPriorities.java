...
public class ResponseTimeAnalysisWithVariousPriorities {
    ...
    private long[][] busyWindow(ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] response_time, int extendCal,
        ...
        for (int i = 0; i < tasks.size(); i++) {
            for (int j = 0; j < tasks.get(i).size(); j++) {
                ...
                response_time_plus[i][j] = oneCalculation(task, tasks, resources, response_time, response_time[i][j], btbHit, useRi);
    ...
    private long oneCalculation(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources,
                                long[][] response_time, long Ri, boolean btbHit, boolean useRi) {
    ...
    private long FIFOPResourceAccessTime(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time,
        ...
        if(task.id == 29){
            System.out.println(cancel_spin);
            task.mrsp_arrivalblocking_overheads = cancel_spin;
        }
...
//        }
        ...
        for (Resource res : longLocalBlockingResource) {
            ...
            local_blocking_each_resource.add(new ArBData(res,local_blocking,remote_processor_size));
        }
...
//            local_blocking_each_resource.sort((l1, l2) -> -Long.compare(l1, l2));
        ...
        return local_blocking_each_resource.size() > 0 ? local_blocking_each_resource.get(0).local_blocking : 0;
    }
