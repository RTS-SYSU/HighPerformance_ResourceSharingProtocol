...
public class ResponseTimeAnalysisWithVariousPriorities_S_B {
...
//        }
    ...
    private long oneCalculation(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] response_time, long Ri,
        ...
        HashMap<Long,ArrayList<ArrayList<Long>>> map = FIFOPResourceAccessTime(task, tasks, resources, response_time, Ri, btbHit, useRi);
        task.spin = map.keySet().iterator().next();
        ArrayList<ArrayList<Long>> requestsLeftOnRemoteP = map.values().iterator().next();
        task.interference = highPriorityInterference(task, tasks, resources, response_time, Ri, oneMig, np, btbHit, useRi);
        task.local = localBlocking(task, tasks, resources, response_time, Ri, oneMig, np, btbHit, useRi, requestsLeftOnRemoteP);
     ...
     */
    private HashMap<Long,ArrayList<ArrayList<Long>>> FIFOPResourceAccessTime(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time,
        ...
        ArrayList<Pair<Pair<Long, Integer>,Integer>> sum_array = new ArrayList<>();
        if(sortedwww.size() > 0) {
            ...
            for(int i = 0 ; i < entryList.size(); i++){
                ...
                ArrayList<Pair<Pair<Long, Integer>,Integer>> sum_list = new ArrayList<>();
                for (Integer index : differenceSet) {
                    for(int wc = 0 ; wc < all_csl_queue.get(index).size(); wc++){
                        sum_list.add(new Pair<>(all_csl_queue.get(index).get(wc),index));
                    }
                }
                ...
                if(sum_array.size() > 1 && sum_array.get(0) != null){
                    ...
                    sum_array.sort(Comparator.comparingLong(pair -> -pair.getKey().getKey()));
                }
        ...
        Long cancel_spin = 0L;
        if(sum_array.size() > 0){
            for(Pair<Pair<Long, Integer>,Integer> pair : sum_array) {
                spin += pair.getKey().getKey();
                cancel_spin += pair.getKey().getKey();
                ArrayList<Long> rp = requestsLeftOnRemoteP.get(pair.getValue());
                ...
                task.implementation_overheads += pair.getKey().getValue() * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
                task.blocking_overheads += pair.getKey().getValue() * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            }
        }
        HashMap<Long,ArrayList<ArrayList<Long>>> map = new HashMap<>();
        map.put(spin,requestsLeftOnRemoteP);
        return map;
    }
    ...
    private long localBlocking(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time, double oneMig,
                               long np, boolean btbHit, boolean useRi, ArrayList<ArrayList<Long>> requestsLeftOnRemoteP) {
        long fifop_localblocking = FIFOPlocalBlocking(t, tasks, resources, Ris, time, btbHit, useRi, requestsLeftOnRemoteP);
    ...
    private long FIFOPlocalBlocking(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long Ri,
                                    boolean btbHit, boolean useRi, ArrayList<ArrayList<Long>> requestsLeftOnRemoteP) {
        ...
        ArrayList<ArBData> local_blocking_each_resource = new ArrayList<>();

        t.fifop_arrivalblocking_overheads = 0;

        for (Resource res : shortLocalBlockingResource) {
            long local_blocking = res.csl;
            local_blocking_each_resource.add(new ArBData(res,local_blocking,0));
        }

        for (Resource res : longLocalBlockingResource) {
            long local_blocking = res.csl;
            if(requestsLeftOnRemoteP.get(res.id - 1).size() > 0){
                local_blocking += requestsLeftOnRemoteP.get(res.id - 1).size() * res.csl;
            }
            local_blocking_each_resource.add(new ArBData(res,local_blocking,requestsLeftOnRemoteP.get(res.id - 1).size()));
...
//            local_blocking_each_resource.add(local_blocking);
        ...
        List<ArBData> sortedList = new ArrayList<>();
        if (local_blocking_each_resource.size() > 0){
            sortedList = local_blocking_each_resource.stream()
        ...
        if (local_blocking_each_resource.size() > 0){
            t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            t.fifop_arrivalblocking_overheads += sortedList.get(0).remote_processor_size * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
        }

        return sortedList.size() > 0 ? sortedList.get(0).local_blocking : 0;
    }
