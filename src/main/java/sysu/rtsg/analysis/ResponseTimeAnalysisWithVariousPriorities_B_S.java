...
public class ResponseTimeAnalysisWithVariousPriorities_B_S {
...
//        }
    ...
    private long oneCalculation(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] response_time, long Ri,
        ...
        LocalBlockingFlag flag = localBlocking(task, tasks, resources, response_time, Ri, oneMig, np, btbHit, useRi);
        task.local = flag.blocking;

        task.spin = FIFOPResourceAccessTime(task, tasks, resources, response_time, Ri, btbHit, useRi, flag.isLongResource, flag.resource);
    ...
    private long FIFOPResourceAccessTime(SporadicTask task, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time,
                                         boolean btbHit, boolean useRi, boolean isABLong, Resource ABResource) {
        // 用于计算资源相关的访问与阻塞时间项
        long spin = 0;
        ...
        if(isABLong){
            ArrayList<Long> rp = requestsLeftOnRemoteP.get(ABResource.id - 1);
     ...
     ************** Arrival Blocking *******************
     ***************************************************/
    private LocalBlockingFlag localBlocking(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long time, double oneMig,
                               long np, boolean btbHit, boolean useRi) {
        LocalBlockingFlag fifop_localblocking = FIFOPlocalBlocking(t, tasks, resources, Ris, time, btbHit, useRi);
    ...
    private LocalBlockingFlag FIFOPlocalBlocking(SporadicTask t, ArrayList<ArrayList<SporadicTask>> tasks, ArrayList<Resource> resources, long[][] Ris, long Ri,
        ...
        // 我们需要真正造成arrival blocking的资源，从而在requestLeftOnRemoteProcessor上进行减少；

        ArrayList<ArBData> local_blocking_each_resource_short = new ArrayList<>();
        ArrayList<ArBData> local_blocking_each_resource_long = new ArrayList<>();

        t.fifop_arrivalblocking_overheads = 0;

        for (Resource res : shortLocalBlockingResource) {
            long local_blocking = res.csl;
            // HashMap<Resource,Long> map = new HashMap<>();
            // map.put(res,local_blocking);
            local_blocking_each_resource_short.add(new ArBData(res,local_blocking,0));
        }

        for (Resource res : longLocalBlockingResource) {
            long local_blocking = res.csl;
            int remote_processor_size = 0;
            for (int parition_index = 0; parition_index < res.partitions.size(); parition_index++) {
                ...
                if (partition != t.partition && (norHP + norT) < norR) {
                    ...
                    remote_processor_size += 1;
                    // t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
                }
            }
            // HashMap<Resource,Long> map = new HashMap<>();
            // map.put(res,local_blocking);
            local_blocking_each_resource_long.add(new ArBData(res,local_blocking,remote_processor_size));
        }
        List<ArBData> sortedListShort = new ArrayList<>();
        List<ArBData> sortedListLong = new ArrayList<>();

        if (local_blocking_each_resource_short.size() > 0){
            sortedListShort = local_blocking_each_resource_short.stream()
                    .sorted(Comparator.comparingLong(arB -> -arB.local_blocking))
                    .toList();
        }

        if (local_blocking_each_resource_long.size() > 0){
            sortedListLong = local_blocking_each_resource_long.stream()
                    .sorted(Comparator.comparingLong(arB -> -arB.local_blocking))
                    .toList();
        }

        // 四种情况
        if(sortedListShort.size() == 0 && sortedListLong.size() == 0){
            return new LocalBlockingFlag(false,null,0L);
        }
        else if(sortedListShort.size() > 0 && sortedListLong.size() == 0){
            t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            t.fifop_arrivalblocking_overheads += sortedListShort.get(0).remote_processor_size * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            return new LocalBlockingFlag(false,sortedListShort.get(0).resource,sortedListShort.get(0).local_blocking);
        }
        else if(sortedListShort.size() == 0 && sortedListLong.size() > 0){
            t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            t.fifop_arrivalblocking_overheads += sortedListLong.get(0).remote_processor_size * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            return new LocalBlockingFlag(true,sortedListLong.get(0).resource,sortedListLong.get(0).local_blocking);
        }
        else if(sortedListShort.size() > 0 && sortedListLong.size() > 0){
            t.fifop_arrivalblocking_overheads += (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
            long shortBlocking = sortedListShort.get(0).local_blocking;
            long longBlocking = sortedListLong.get(0).local_blocking;
            // 如果相等，在最坏情况下我们应让短资源造成arrival blocking
            if(shortBlocking >= longBlocking){
                t.fifop_arrivalblocking_overheads += sortedListShort.get(0).remote_processor_size * (AnalysisUtils.FIFOP_LOCK + AnalysisUtils.FIFOP_UNLOCK);
                return new LocalBlockingFlag(false,sortedListShort.get(0).resource,sortedListShort.get(0).local_blocking);
            }
            else{
